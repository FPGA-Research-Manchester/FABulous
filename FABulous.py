...
To run the complete FABulous flow with the default project, run the following command:
    ...
    def do_place_and_route_npnr(self, args):
        ...
        top_module_name = json_file.split('.')[0]
        ...
        if os.path.exists(f"{self.projectDir}/{path}"):
            ...
            if f"{json_file}" in os.listdir(f"{self.projectDir}/{path}"):
                runCmd = [f"FAB_ROOT={self.projectDir}",
                          ...
                          "-o", f"fasm={self.projectDir}/{path}/{top_module_name}.fasm",
                          "--verbose",
                          "--log", f"{self.projectDir}/{path}/{top_module_name}_npnr_log.txt"]
    ...
    def do_run_FABulous_bitstream(self, *args):
        ...
        if args[0] == "vpr":
            ...
            json_file = args[1].split('.')[0] + ".json"
            self.do_place_and_route_npnr(json_file)
            self.do_gen_bitStream_binary(args[1])
