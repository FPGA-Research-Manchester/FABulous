...
To generate the bitstream, the user can call the ``gen_bitstream_binary <design_des.fasm>`` command from the CLI, where the
``design_des.fasm`` is the ``.fasm`` file generated by synthesis and place and route.
