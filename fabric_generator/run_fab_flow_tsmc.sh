...
python3 fabric_gen.py -GenBitstreamSpec npnroutput/meta_data_tsmc.txt
#GOTO END

mkdir -p verilog_output_tsmc
mv -f *.v ./verilog_output_tsmc

mkdir -p vhdl_output_tsmc
mv -f *.vhdl ./vhdl_output_tsmc
...
cp ./npnroutput/{bel.txt,pips.txt,template.v,meta_data_tsmc.txt} ../nextpnr/fabulous/fab_arch/
