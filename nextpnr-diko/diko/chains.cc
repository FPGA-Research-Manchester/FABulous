...
class ChainConstrainer
{
  private:
    ...
    CellInfo *make_carry_pass_out(PortInfo &cout_port, CellInfo *cin_cell = nullptr)
    {
        ...
        std::unique_ptr<CellInfo> lc = create_diko_cell(ctx, ctx->id("diko_LC"));
        lc->params[ctx->id("INIT")] = "65280"; // 0xff00: O = I3
...
//This is the xilinx implementation just in case you need it
...
//     {
//         NPNR_ASSERT(cout_port.net != nullptr);
//         std::unique_ptr<CellInfo> lc = create_diko_cell(ctx, ctx->id("ICESTORM_LC"));
