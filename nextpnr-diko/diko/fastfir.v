...
//`endif
	...
	/*input*/	wire			i_clk, i_reset;
        (* keep *) LUT1 i_clk_in(.O(i_clk));
        (* keep *) LUT1 i_reset_in(.O(i_reset));
	//
	/*input*/	wire			i_tap_wr;	// Ignored if FIXED_TAPS
        (* keep *) LUT1 i_tap_wr_in(.O(i_tap_wr));
genvar i;
	/*input*/	wire	[(TW-1):0]	i_tap;		// Ignored if FIXED_TAPS
generate for (i = 0; i < TW; i++) 
        (* keep *) LUT1 i_tap_in(.O(i_tap[i]));
endgenerate
	//
	/*input*/	wire			i_ce;
        (* keep *) LUT1 i_ce_in(.O(i_ce));
	/*input*/	wire	[(IW-1):0]	i_sample;
generate for (i = 0; i < IW; i++) 
        (* keep *) LUT1 i_sample_in(.O(i_sample[i]));
endgenerate
	/*output*/	wire	[(OW-1):0]	o_result;
generate for (i = 0; i < OW; i++) 
        (* keep *) LUT1 o_result_out(.I0(o_result[i]));
endgenerate
